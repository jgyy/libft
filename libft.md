...
- [Chapter III: AI Instructions](#chapter-iii-ai-instructions)
...
- [Chapter V: Readme Requirements](#chapter-v-readme-requirements)
...
| **Description**       | Allocates memory (using `malloc(3)`) and returns an array of strings obtained by splitting `s` using the character `c` as a delimiter.<br>Each string in the returned array is allocated independently.<br>The array of pointers itself is also allocated dynamically.<br>The returned array must be `NULL` terminated. |
...
- `content`: The data contained in the node.<br>Using `void *` allows you to store any type of data.
...
| **Return Value**      | None                                                                                            |
| **External Function** | `free`                                                                                          |
| **Description**       | Deletes and frees the given node and all its successors, using the function `del` and `free(3)`.<br>Finally, set the pointer to the list to `NULL`. |
